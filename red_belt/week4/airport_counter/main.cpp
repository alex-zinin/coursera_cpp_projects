...
public:
  ...
  {
      size_t i = 0;
      //vector<size_t> v(static_cast<uint32_t>(TAirport::Last_));
      for(auto it = database.begin(); it < database.end(); it++)
      {
          it->first = static_cast<TAirport>(i);
          i++;
      }
      for(auto it = begin; it < end; it++)
      {
         // database[static_cast<uint32_t>(*it)].first = *it;
          database[static_cast<uint32_t>(*it)].second++;
      }
  }
  ...
  {//cout<< "qqq"<<endl;
      return database[static_cast<uint32_t>(airport)].second;
  }

  // добавить данный элемент
  void Insert(const TAirport& airport)
  {//cout<< "qqq"<<endl;
        database[static_cast<uint32_t>(airport)].second++;
  }
  ...
  {//cout<< "qqq"<<endl;
      database[static_cast<uint32_t>(airport)].second = 0;
  }
