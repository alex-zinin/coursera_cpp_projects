...
private:
  std::vector<T> coeffs_ = {0};
    ...
    {
    public:
        IndexProxy(Polynomial& pol, size_t degr): degree(degr), poly(pol) {}

        T& operator=(T rhs)
        {
            while(degree >= poly.coeffs_.size())
                poly.coeffs_.push_back(0);
            poly.coeffs_[degree] = rhs;
            poly.Shrink();
            return poly.coeffs_[degree];
        }
