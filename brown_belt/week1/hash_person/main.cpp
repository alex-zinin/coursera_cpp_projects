...
struct AddressHasher {
    ...
    {
        size_t s1 = shash(address.city);
        size_t s2 = shash(address.street);
        size_t s3 = ihash(address.building);
        int x = 14;
        return s1 + s2*x + s3*x*x;
    }
...
struct PersonHasher {
    ...
    {
        size_t s1 = shash(person.name);
        size_t s2 = dhash(person.weight);
        size_t s3 = ihash(person.height);
        size_t s4 = ahash(person.address);
        int x = 17;
        return s1 + s2*x + s3*x*x + s4*x*x*x;
    }
